...
import type { MunaqasyahRequest } from './MunaqasyahRequestTable';
...
export function ExportToPDFButton({ table, academicYear }: Props) {
  ...
  const handleExport = () => {
    ...
    const formattedData = filteredRows.map((row, index) => {
      ...
      const batch = batchLabels[d.batch];

      const stage = stageLabels[d.stage];

      const juz = d.juz.name;
    ...
    doc.text('LAPORAN PERMINTAAN MUNAQASYAH SISWA', doc.internal.pageSize.width / 2, 28, {
      align: 'center',
    });
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        4: { halign: 'center', cellWidth: 35 }, // Guru Pembimbing
        5: { halign: 'center', cellWidth: 25 }, // Batch
        6: { halign: 'center', cellWidth: 25 }, // Tahapan
        7: { halign: 'center', cellWidth: 25 }, // Juz
    ...
    const filename = `laporan-permintaan-munaqasyah-${timestamp}.pdf`;
