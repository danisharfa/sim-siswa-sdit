...
export function ClassroomMembersAlertDialog({
  member,
  classroomId,
  open,
  onOpenChange,
  ...
  async function handleAction() {
    ...
    try {
      ...
      toast.success(data.message || 'Siswa berhasil dihapus dari kelas');
      onConfirm();
      onOpenChange(false);
    } catch (error) {
      console.error(error);
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Hapus Siswa Kelas</AlertDialogTitle>
          <AlertDialogDescription>
            Apakah Anda yakin ingin menghapus siswa ini dari kelas? Tindakan ini tidak dapat
            dibatalkan.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
