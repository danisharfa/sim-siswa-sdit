...
interface StudentTashihSchedule {
  id: string;
  date: string;
  sessionName: string;
  startTime: string;
...
interface StudentTashihScheduleTableProps {
  data: StudentTashihSchedule[];
}

export function StudentTashihScheduleTable({ data }: StudentTashihScheduleTableProps) {
  const {
    sorting,
    setSorting,
    columnFilters,
    ...
    columnVisibility,
    setColumnVisibility,
  } = useDataTableState<StudentTashihSchedule, string>();
  ...
  const columns = useMemo<ColumnDef<StudentTashihSchedule>[]>(
    () => [
      {
        accessorKey: 'date',
        id: 'Tanggal',
