...
import type { TashihSchedule } from '@/components/teacher/tashih/schedule/TashihScheduleTable';
...
export function ExportToPDFButton({ table, teacherName, academicYear }: Props) {
  const handleExport = () => {
    ...
    doc.text('LAPORAN JADWAL TASHIH SISWA BIMBINGAN', doc.internal.pageSize.width / 2, 28, {
      align: 'center',
    });
    ...
    // Table Section
    autoTable(doc, {
      head: [['No', 'Tanggal', 'Sesi & Lokasi', 'Siswa', 'Kelompok', 'Jenis Tashih', 'Materi']],
      body: formattedData,
      startY: lineY + 8,
      styles: {
        fontSize: 8,
      ...
      columnStyles: {
        ...
        4: { halign: 'center', cellWidth: 40 }, // Kelompok
        5: { halign: 'center', cellWidth: 30 }, // Jenis Tashih
        6: { halign: 'center', cellWidth: 55 }, // Materi
      },
      alternateRowStyles: {
    ...
    const filename = `laporan-jadwal-tashih-guru-${timestamp}.pdf`;
