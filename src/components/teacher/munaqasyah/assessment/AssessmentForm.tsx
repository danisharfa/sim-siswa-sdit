...
export function AssessmentForm({ onSaved }: { onSaved: () => void }) {
  ...
  const tasmiAverage = useMemo(() => {
    ...
    const percentages = tasmiRows.map((row) => calculateTasmiPercentage(row));
    return percentages.reduce((acc, p) => acc + p, 0) / percentages.length;
  }, [tasmiRows]);

  const munaAverage = useMemo(() => {
    const percentages = munaqasyahRows.map((row) => calculateMunaqasyahPercentage(row));
                  ...
                  {selectedRequest.stage === MunaqasyahStage.TASMI ? (
                    ...
                    <div className="space-y-3">
                      ...
                      {munaqasyahRows.map((row, idx) => {
                        const total = calculateMunaqasyahRawTotal(row);
                        const percent = calculateMunaqasyahPercentage(row);
                        return (
                          <div
                            key={row.questionNo}
